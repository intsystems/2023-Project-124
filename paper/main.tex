...
Для этого рассмотрим $T_\mathbf{\omega} = g_f(V_\mathbf{\omega})$, где $V_\omega : \mathcal{X} \to \mathbb{R}$, а $g_f: \mathbb{R} \to \text{dom}_{f^*}$ -- функция активации, подходящая для выбранной дивергенции.
...
\begin{equation}
	f_{H^2}(u) = (\sqrt{u} - 1)^2, ~~~~~ f^*_{H^2}(t) = \frac{t}{1 - t}.
\end{equation}
$T$ параметризуем через выход дискриминатора $D_{H^2}(x) = \dfrac{1}{1-\exp(-V)}$, взяв функцию активации:
\begin{equation}
	g_{f_{H^2}} = 1 - \exp(V) = \frac{1}{1 - D(x)}.
\end{equation}
Итого получаем следующую оптимизационную задачу в терминах, введенных для диффузионной модели:
 \begin{equation}
	\min\limits_\theta\max\limits_\varphi\sum\limits_{t\geqslant 1}^n \mathbb{E}_{q(\mathbf{x_t})}[\mathbb{E}_{q(\mathbf{x_{t-1}}|\mathbf{x_t})}[\frac{1}{1 - (D_\varphi(\mathbf{x_{t-1}}, \mathbf{x_t}, t) ) }] + \mathbb{E}_{p_\theta(\mathbf{x_{t-1}}|\mathbf{x_t})}[\frac{1}{D_\varphi(\mathbf{x_{t-1}}, \mathbf{x_t}, t)}]].
\end{equation}\\

TO DO: ФУКНЦИИ ДЛЯ ДИСКРИМИНАТОРА И ГЕНЕРАТОРА \\
...
Порождающая функция и ее сопряженная:
\begin{equation}
	f_{R-KL}(u) = -\log{u}, ~~~~~ f^*_{R-KL}(t) = -1- \log({-t}).
\end{equation}
$T$ параметризуем таким же образом со следующей функцией активации:
\begin{equation}
	g_{f_{R-KL}} = - \exp(V) = \frac{D(x)}{1 - D(x)}.
\end{equation}
Минимизационная задача для диффузионной модели с исопльзованием минимизации обратной KL-дивергенции:
\begin{equation}
	\min\limits_\theta\max\limits_\varphi\sum\limits_{t\geqslant 1}^n \mathbb{E}_{q(\mathbf{x_t})}[\mathbb{E}_{q(\mathbf{x_{t-1}}|\mathbf{x_t})}[\frac{D_\varphi(\mathbf{x_{t-1}}, \mathbf{x_t}, t)}{1 - D_\varphi(\mathbf{x_{t-1}}, \mathbf{x_t}, t)}] + \mathbb{E}_{p_\theta(\mathbf{x_{t-1}}|\mathbf{x_t})}[\log{\left(-\frac{D_\varphi(\mathbf{x_{t-1}}, \mathbf{x_t}, t)}{1 - D_\varphi(\mathbf{x_{t-1}}, \mathbf{x_t}, t)}\right)}]].
\end{equation}\\
