...
class Discriminator(nn.Module):
    def __init__(self, time_emb_dim=100, in_channels=1, n_steps=4, device=None):
        super(Discriminator, self).__init__()
        self.device = device
        self.emb_matr = sinusoidal_embedding(n_steps, time_emb_dim).to(self.device)

        # First half
        self.te1 = self._make_te(time_emb_dim, in_channels*2)
        self.b1 = nn.Sequential(
            DoubleConv((in_channels*2, 28, 28), in_channels*2, 15),
    ...
    def forward(self, x_t, t_in, x_tp1):

        t = F.embedding(t_in, self.emb_matr)
        ...
        out = self.fc_out(self.flatten(out_mid))
...
class Generator(nn.Module):
    def __init__(self, time_emb_dim=100, in_channels=1,
                 out_channels=1, n_steps=4, device=None, zsize=100):
        ...
        self.z_transform = nn.Sequential(
            PixelNorm(),
            nn.Flatten(),
            nn.Linear(zsize, 28 * 28 * 1),
            nn.SiLU(),
            nn.Linear(28 * 28 * 1, 28 * 28 * 1),
            nn.SiLU(),
            nn.Linear(28 * 28 * 1, 28 * 28 * 1),
            nn.SiLU(),
            nn.Linear(28 * 28 * 1, 28 * 28 * 1),
    ...
    def forward(self, x_t, t, latent_z):
        t = F.embedding(t, self.emb_matr)
        t.requires_grad_(False)

        zemb = self.z_transform(latent_z)
        x = torch.cat((x_t, zemb.reshape(-1, 1, 28, 28)), dim=1)
